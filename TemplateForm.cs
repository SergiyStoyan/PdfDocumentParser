    ...
    public partial class TemplateForm : Form
    {
            ...
            picture.MouseUp += delegate (object sender, MouseEventArgs e)
            {
                ...
                if (!cSelectAnchor.Checked)
                {
                    if (cSetPosition0Anchor.Checked)
                    {
                        ...
                        template.Position0Anchor = new ImageData(b);
                    ...
                    switch (mode)
                    {
                        ...
                        case Modes.SetFieldRectangle:
                            ...
                            break;
                        //case Modes.SetPageRecognitionTextMarks:
                        //    {
                        //        if (fields.SelectedRows.Count < 1)
                        //            break;
                        //        string s = (string)fields.SelectedRows[0].Cells["PageRecognitionTextMarks"].Value;
                        //        if (s == null)
                        //            s = "";
                        //        List<Settings.Template.Mark> tms = SerializationRoutines.Json.Deserialize<List<Settings.Template.Mark>>(s);
                        //        if (tms == null)
                        //            tms = new List<Settings.Template.Mark>();
                        //        tms.Add(new Settings.Template.Mark
                        //        {
                        //            Rectangle = new Settings.Template.RectangleF { X = p1.X, Y = p2.Y, Width = p2.X - p1.X, Height = p2.Y - p1.Y },
                        //            Text = pdfReader.ExtractText(currentPage, p1.X, pdf_h - p2.Y, p2.X - p1.X, p2.Y - p1.Y),
                        //        });
                        //        fields.SelectedRows[0].Cells["PageRecognitionTextMarks"].Value = SerializationRoutines.Json.Serialize(tms);
                        //    }
                        //    break;
                        default:
                            break;
        ...
        public Settings.Template Template
        {
            set
            {
                ...
                if (template.Position0Anchor != null)
                    position0AnchorRectangular.Text = "{ " + template.Position0Anchor.Width + " x " + template.Position0Anchor.Height + " }";
        ...
        void showPage(int page_i)
        {
            try
            {
                ...
                if (template.Position0Anchor != null)
                {
                    PointF? p_ = template.Position0Anchor.FindWithinImage(new ImageData(pageBitmaps.Get(currentPage)));
                    if (p_ != null)
                    {
                        PointF p = (PointF)p_;
                        drawBox(Color.Green, p.X, p.Y, template.Position0Anchor.Width, template.Position0Anchor.Height);
                    }
                }
